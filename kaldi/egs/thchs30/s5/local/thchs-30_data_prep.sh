...
for x in train dev test; do
  ...
  for nn in `find  $corpus_dir/$x -name "*.wav" | sort -u | xargs -I {} basename {} .wav`; do
      spkid=`echo $nn | awk -F"_" '{print "" $1}'`                        # -F:field, split flag
      spk_char=`echo $spkid | sed 's/\([A-Z]\).*/\1/'`                    # \(:转义字符
      ...
      echo $uttid `sed -n 1p $corpus_dir/data/$nn.wav.trn` >> word.txt    # 1st line
      echo $uttid `sed -n 3p $corpus_dir/data/$nn.wav.trn` >> phone.txt   # 3rd line
  done
  cp word.txt text
